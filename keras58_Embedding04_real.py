...
wordl_size = len(token.word_index)
print(wordl_size) #=> input_dim을 알기 위해 언어사전개수 확인 / 거기에 +1

# 2 Model
model=Sequential()
#=========================
model.add(Embedding(input_dim=27,output_dim=10,input_length=5))
...
y_predict=np.around(model.predict(x))
print("loss:",result)
print(y_predict)