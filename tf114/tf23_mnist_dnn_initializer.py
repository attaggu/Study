...
w3 = tf.compat.v1.Variable(tf.random_normal([64,32], name='weight3')) # (n,9) 에 (9,8)를 곱해 (n,8)
...
w4 = tf.compat.v1.Variable(tf.random_normal([32,16], name='weight4')) # (n,9) 에 (9,8)를 곱해 (n,8)
...
w5 = tf.compat.v1.Variable(tf.random_normal([16,10], name='weight5')) # (n,9) 에 (9,8)를 곱해 (n,8)
...
for step in range(epochs):
    cost_val, _, w_val, b_val = sess.run([loss,train,w5,b5],
                                         feed_dict={x:x_train, y:y_train,keep_prob:0.66})
    if step %20 == 0:
